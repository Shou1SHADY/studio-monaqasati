...
`pmHandovers` (PM 1.0 — the handover FILE from CRM: `wait → acc | ret`, addressed to one manager `to`; it creates no project — the addressed manager accepts it in "New projects" (`/contractor/projects/inbox`), which creates the project with `pmHandoverId` and a `pm` block {no PJ-yyyy/NNN, lifecycle plan/live/hold/done/closed, terms, original (frozen at Start), startOn, durationDays}) · `pmEvents` (PM → Finance outbox, doc id = idempotency key `prj:ADV:<no>` etc. with `/`→`_`, append-only) · `invoices` · `rfqShareLinks` · `guestOfferLinks`

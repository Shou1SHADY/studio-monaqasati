...
export default function SupplierWarehousesPage() {
  ...
  return (
    <PortalLayout>
            ...
            {list.map((wh) => (
              <Card key={wh.id} className="hover:shadow-md transition-shadow">
                <CardContent className="p-5">
                  ...
                  {wh.description && <p className="text-xs text-muted-foreground/70 truncate">{wh.description}</p>}
                  <Link
                    href={`/supplier/warehouses/${wh.id}`}
                    className={cn(
      ...
      {editWarehouse && (
        <WarehouseDialog open={!!editWarehouse} onOpenChange={(open) => { if (!open) setEditWarehouse(null) }}
          warehouse={editWarehouse} orgId={myOrgId} t={t} locale={locale} />
      )}
      ...
      <AlertDialog open={!!deleteWarehouse} onOpenChange={(open) => { if (!open) setDeleteWarehouse(null) }}>
        <AlertDialogContent dir={isRtl ? "rtl" : "ltr"}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("wh_delete_confirm_desc", { name: deleteWarehouse?.name ?? "" })}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

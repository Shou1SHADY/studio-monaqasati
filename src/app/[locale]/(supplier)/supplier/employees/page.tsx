      ...
      <DialogContent dir={locale === "ar" ? "rtl" : "ltr"}>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="emp-salary" type="number" min="0" value={salary}
              onChange={(e) => setSalary(e.target.value)} placeholder={t("emp_salary_placeholder")} dir="ltr" />
          </div>
        </div>
        <DialogFooter>
...
export default function SupplierEmployeesPage() {
  ...
  return (
    <PortalLayout>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1 justify-end">
                        <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-primary"
                          onClick={() => setEditEmployee(emp)} aria-label={t("emp_edit_title")}>
                          <Pencil size={13} />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive"
                          onClick={() => setDeleteEmployee(emp)} aria-label={t("emp_delete_btn")}>
                          <Trash2 size={13} />
                        </Button>
                      </div>
                    </td>
      ...
      <EmployeeDialog open={showAdd} onOpenChange={setShowAdd} orgId={myOrgId} t={t} locale={locale} />
      {editEmployee && (
        <EmployeeDialog open={!!editEmployee} onOpenChange={(open) => { if (!open) setEditEmployee(null) }}
          employee={editEmployee} orgId={myOrgId} t={t} locale={locale} />
      )}
      ...
      <AlertDialog open={!!deleteEmployee} onOpenChange={(open) => { if (!open) setDeleteEmployee(null) }}>
        <AlertDialogContent dir={isRtl ? "rtl" : "ltr"}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("emp_delete_confirm_desc", { name: deleteEmployee?.name ?? "" })}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting}>{t("emp_cancel")}</AlertDialogCancel>
            <AlertDialogAction onClick={handleDelete} disabled={isDeleting} className="bg-destructive hover:bg-destructive/90 gap-2">

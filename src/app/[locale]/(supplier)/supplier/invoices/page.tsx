        ...
        <div className="space-y-5 py-2">
                ...
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-5">
                    <Input value={item.description}
                      ...
                      placeholder={t("inv_item_desc_placeholder")} className="h-8 text-sm" />
                  </div>
                  ...
                  <div className="col-span-1 flex justify-end">
                    <Button type="button" variant="ghost" size="icon"
                      ...
                      onClick={() => removeItem(idx)} aria-label={t("inv_remove_item")}>
                      <X size={13} />
                    </Button>
                  </div>
                </div>
          ...
          <div className="border-t pt-4 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between font-bold border-t pt-1.5">
              ...
              <span dir="ltr" className="font-mono">{formatCurrency(subtotal + vat, locale)}</span>
            </div>
          </div>
...
export default function SupplierInvoicesPage() {
  ...
  return (
    <PortalLayout>
          ...
          {STATUS_TABS.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={cn(
                ...
                activeTab === tab ? "bg-primary text-white" : "bg-muted/30 text-muted-foreground hover:bg-muted/60"
              )}>
              {t(`inv_tab_${tab}`)}
                  ...
                  <CardContent className="p-5">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-muted/40">
                      ...
                      <Button size="sm" variant="ghost" className="gap-1 h-7 text-xs text-muted-foreground"
                        ...
                        <Pencil size={12} />{t("inv_edit_title")}
                      </Button>
                      {inv.status === "draft" && (
                        <Button size="sm" variant="ghost"
                          className="gap-1 h-7 text-xs text-muted-foreground hover:text-destructive"
                          onClick={() => handleDelete(inv.id)} disabled={isDeleting === inv.id}>
      ...
      {editInvoice && (
        <InvoiceDialog open={!!editInvoice} onOpenChange={(open) => { if (!open) setEditInvoice(null) }}
          invoice={editInvoice} orgId={myOrgId} t={t} locale={locale} />
      )}
    </PortalLayout>
  )
}

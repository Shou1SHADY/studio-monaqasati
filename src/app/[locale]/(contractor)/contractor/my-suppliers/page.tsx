...
import { Loader2, Users, CheckCircle, XCircle, LinkIcon, Calendar } from "lucide-react"
...
export default function MySuppliersPage() {
  ...
  const linksQuery = useMemoFirebase(() => {
    ...
    return query(
      ...
      where("contractorOrgId", "==", typedProfile?.organizationId || user.uid)
    )
                      ...
                      <CardContent className="p-5" dir={isRtl ? "rtl" : "ltr"}>
                        ...
                        <div className="flex items-start justify-between gap-2 mb-3">
                          ...
                          >
                            <Calendar size={12} />
                            <span>
                              {t("my_sup_requested_at")}: {fmtDate(link.requestedAt, locale)}
                            </span>
                          </div>
                        </div>
                          ...
                          <div className="mb-4">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {link.categories.map((cat) => (
                                <Badge key={cat} variant="secondary" className="text-[11px] bg-white text-slate-600 border border-slate-200">
                                  {cat}
                                </Badge>
                              ))}
                            </div>
                          ...
                          <div>
                            ...
                            <div
                              className={cn(
                                ...
                                isRtl ? "flex-row-reverse ps-10" : "ms-10"
                              )}
                            >
                              <Calendar size={11} />
                              <span>
                                {t("my_sup_connected_since")}: {fmtDate(link.connectedAt, locale)}
                              </span>
                            </div>
                          </div>

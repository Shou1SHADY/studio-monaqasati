...
import { displayPoNumber, displayReceiptNumber, PROC_DOC_PREFIXES } from "@/lib/procurement/format"
import { displayDocNumber } from "@/lib/sales-numbering"

describe("Procurement document numbers", () => {
  it("PO → ط.ش and GR → ا.س in Arabic, untouched in English; Sales' prefixes still work", () => {
    ...
    expect(PROC_DOC_PREFIXES).toEqual({ PO: { latin: "PO", arabic: "ط.ش" }, GR: { latin: "GR", arabic: "ا.س" } })
  })
})
